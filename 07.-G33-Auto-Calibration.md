...
`Zx.xx` : Sets the Z-offset. Every subsequent G33 will use the last Z-offset. (Marlin 1.1.7 upwards)  

**Examples**  
`G33` : calibrates with the default settings.  
`G33 Z0.03` : sets the z-offset to 0.03 mm.  
...
3) Send the command `G33 Z0.0` (sets Z-offset to 0.0).
...
7) Send `G33 Z0.xx` to set your Z-offset.
8) `M500` to save.
...
2) Via command `G33 Zx.x`.
For example: `G33 Z0.73`
3) (Only on versions prior to 1.1.7) Via the LCD menu. Control -> Motion -> Z-offset.
